...
> 本目录包含 CraftFlow 设置系统、适配器架构和流式输出的设计文档。执行时以 `implementation-plan.md` 为主线，其余文档作为设计参考。
...
implementation-plan.md          ← 执行主线：什么时候做、按什么顺序做
    ...
    └──→ streaming-architecture.md    ← 独立设计：LLM 对话流式输出方案、双端架构、SSE 协议
```
...
| [streaming-architecture.md](streaming-architecture.md) | 流式输出架构：SSE 协议选型、双端数据流、LLM 对话功能设计 | 实现对话功能时 |
