...
EDITOR_MAX_TOKENS: int = 1024         # EditorNode: JSON 评分 ~300-500 token，1024 足够
FACTCHECKER_MAX_TOKENS: int = 2048    # FactCheckerNode: 核查报告 JSON ~300-650 token
...
def get_editor_llm() -> BaseChatModel:
    """获取编辑节点专用 LLM 实例（低温度 + 小 max_tokens）

    编辑节点输出结构化 JSON 评分（~300-500 token），
    低温度确保输出更保守确定，小 max_tokens 避免过度生成。
