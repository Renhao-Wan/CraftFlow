...
class TestRouterNode:
    ...
    async def test_router_with_llm_recommendation(self):
        ...
        with patch(
            "app.graph.polishing.nodes.get_default_llm",
...
class TestFactCheckerNode:
    ...
    async def test_fact_checker_success(self):
        ...
        with patch(
            "app.graph.polishing.nodes.get_factchecker_llm",
            new_callable=AsyncMock,
            return_value=mock_llm,
        ):
            state: PolishingState = {
...
class TestAuthorNode:
    ...
    async def test_author_success(self):
        ...
        with patch(
            "app.graph.polishing.debate.nodes.get_default_llm",
            new_callable=AsyncMock,
            return_value=mock_llm,
        ):
            state: DebateState = {
...
class TestEditorNode:
    ...
    async def test_editor_success(self):
        ...
        with patch(
            "app.graph.polishing.debate.nodes.get_editor_llm",
            new_callable=AsyncMock,
            return_value=mock_llm,
        ):
            state: DebateState = {
